...
//9. Complementary filter state
typedef struct
{
	float pitch;  // filtered angle (degrees)
	float roll;  // filtered angle (degrees)
	float alpha;  // trust weight for gyro (0.0–1.0, typically ~0.98)
}CompFilter_Def;
...
//15- Complementary Filter Init (set alpha, zero the angle)
void MPU6050_CompFilter_Init(CompFilter_Def *cf, float alpha);
//16- Complementary Filter Update (call at fixed dt in seconds; reads accel+gyro internally)
